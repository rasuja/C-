...
// 1. データを保存するリスト（今回はToDoクラスのリストにします）
// トップレベル（Program.csの直下）では static を書かなくても、アプリ起動中はデータが保持されます
...
app.MapPost("/add", async (HttpContext context) =>
{
    ...
    {
        // リストに新しいToDoを追加
        todoList.Add(new ToDo(taskname, false));
    }
    // "/"（トップ画面）にリダイレクトして再表示
...
app.MapPost("/done", async (HttpContext context) =>
{
    ...
    if (index >= 0 && index < todoList.Count)
    {
        todoList[index] = todoList[index] with { IsDone = true };
